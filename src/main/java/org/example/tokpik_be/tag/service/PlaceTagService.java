...
public class PlaceTagService {
    ...
    public UserPlaceTagResponse updateUserPlaceTags(long userId, UserPlaceTagsRequest request) {
        ...
        for (long tagId : request.placeTagIds()) {
            ...
            UserPlaceTag userPlaceTag = new UserPlaceTag(user.getId(),
                placeTagRepository.findById(tagId).get());
        ...
        List<UserPlaceTagResponse.PlaceTagDTO> placeTagDTOList = updatedTags.stream()
            .map(tag -> new UserPlaceTagResponse.PlaceTagDTO(tag.getPlaceTag().getId(),
                tag.getPlaceTag().getContent()))
            .toList();
    ...
    public PlaceTagTotalResponse getAllPlaceTags() {
        List<PlaceTag> placeTags = placeTagRepository.findAll();
        List<PlaceTagResponse> responses = placeTags.stream().map(PlaceTagResponse::from).toList();

        return new PlaceTagTotalResponse(responses);
    }
}
